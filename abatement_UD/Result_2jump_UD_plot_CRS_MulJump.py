    ...
    for id_psi0 in range(len(psi0arr)):
        for id_psi1 in range(len(psi1arr)):
                ...
                if xiaarr[id_xiag]>10:
                    ...
                    plt.plot(res["years"], res["distorted_composite_prob"],label='baseline',linewidth=5.0)
                else:
                    plt.plot(res["years"], res["distorted_composite_prob"],label='$\\xi_p={:.5f}$,$\\xi_m={:.3f}$' .format(xiaarr[id_xiag],xigarr[id_xiag]) ,linewidth=5.0)
                # plt.plot(res2["years"], res2["distorted_damage_prob"],label=r'$\xi_p=\\xi_g=0.050$',linewidth=7.0)
                # plt.plot(res3["years"], res3["distorted_damage_prob"],label='baseline',linewidth=7.0)
                plt.xlabel('Years')
                plt.title("Distorted probability of composite")
                plt.ylim(0,1)
                plt.xlim(0,IntPeriod)
                plt.legend(loc='upper left')
    ...
    for id_psi0 in range(len(psi0arr)):
        for id_psi1 in range(len(psi1arr)):
                ...
                if xiaarr[id_xiag]>10:
                    ...
                    plt.plot(res["years"], res["true_composite_prob"],label='baseline',linewidth=5.0)
                else:
                    plt.plot(res["years"], res["true_composite_prob"],label='$\\xi_p={:.5f}$,$\\xi_m={:.3f}$' .format(xiaarr[id_xiag],xigarr[id_xiag]) ,linewidth=5.0)

                plt.xlabel("Years")
                plt.title("True probability of composite")
                plt.ylim(0,1)
                plt.xlim(0,IntPeriod)
                plt.legend(loc='upper left')
