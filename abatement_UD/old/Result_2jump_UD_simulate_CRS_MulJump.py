...
def simulate_pre(
    ...
    ii, ee, xx, g_composite, pi_c, v = controls
    ME_base = ME
    n_bar = n_bar
    K_0, Y_0, L_0 = initial
    ...
    ee = ee[:,:n_bar+1,:]
    xx = xx[:,:n_bar+1,:]
    g_composite = g_composite[:,:,:n_bar+1,:]
    pi_c = pi_c[:,:,:n_bar+1,:]
    v = v[:,:n_bar+1,:]
    ...
    n_composite = len(g_composite)

    composite_func_list = []
    for i in range(n_composite):
        func_i = RegularGridInterpolator(gridpoints, g_composite[i])
        composite_func_list.append(func_i)
    ...
    gt_cmp    = np.zeros([n_composite, pers])
    ...
    for tm in range(pers):
        if tm == 0:
            ...
            for i in range(n_composite):
                composite_func = composite_func_list[i]
                gt_cmp[i, 0] = composite_func(hist[0, :])
            ...
            #     gt_cmp[i, tm] = damage_func(hist[tm-1, :])
            for i in range(n_composite):
                composite_func = composite_func_list[i]
                gt_cmp[i, tm] = composite_func(hist[tm-1, :])
    ...
    # distorted_damage_intensity = np.mean(gt_cmp, axis=0) * damage_intensity
    ...
    damage_intensity = Damage_Intensity(hist[:, 1])
    tech_intensity   = np.exp(hist[:, 2])/448
    
    distorted_composite_intensity = np.mean(gt_cmp, axis=0) * (damage_intensity + tech_intensity)
    
    distorted_composite_prob = 1 - np.exp(- np.cumsum(np.insert(distorted_composite_intensity * dt, 0, 0) ))[:-1]
    
    true_composite_intensity =  damage_intensity + tech_intensity
    true_composite_prob = 1 - np.exp(- np.cumsum(np.insert(true_composite_intensity * dt, 0, 0) ))[:-1]
    ...
    res = dict(
        ...
        distorted_composite_prob=distorted_composite_prob,
        pic_t = pi_c_t,
        ...
        true_composite_prob=true_composite_prob,
...
def model_simulation_generate(xi_a,xi_g,psi_0,psi_1):
    ...
    g_composite = model_tech1_pre_damage_ME_base["g_composite"]
    ...
    res = simulate_pre(grid = (K, Y_short, L), 
                    ...
                    #    controls = (i,e,x, g_tech, g_damage, pi_c, v),
                       controls = (i,e,x, g_composite, pi_c, v),
                       ME = ME_family,
                       n_bar = n_bar,  
                       T0=0, 
                       T=IntPeriod, 
