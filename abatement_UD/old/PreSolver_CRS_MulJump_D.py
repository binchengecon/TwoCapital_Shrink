...
def hjb_pre_tech(
    ...
    while FC_Err > tol and epoch < max_iter:
        ...
        if model == "Pre damage":
            ...
            g_m = np.exp(- 1 / xi_g * ( v_i - v0 + V_post_tech - v0 ))

            D += xi_g * (Intensity + np.exp(  L_mat - np.log(448) ) ) * np.sum( pi_d_o*(1-g_m+g_m*np.log(g_m)),axis=0) + (Intensity + np.exp(  L_mat - np.log(448) ) ) * np.sum(pi_d_o*g_m* ( v_i + V_post_tech),axis=0)
            A += -2 * (Intensity + np.exp(  L_mat - np.log(448) ) ) * np.sum(pi_d_o * g_m,axis=0)

            # D += xi_g * (Intensity + np.exp(  L_mat - np.log(448) ) ) * (1 - np.sum( pi_d_o * np.exp( - xi_g * (v_i - v0 + V_post_tech - v0) ) ) )
        elif model == "Post damage":
            
            gg = np.exp(-1 / xi_g * (V_post_tech - v0))
            gg[gg <=1e-16] = 1e-16
            
            # D += xi_g * np.exp((L_mat - np.log(448))) * (1 - gg + gg * np.log(gg)) + np.exp( (L_mat - np.log(448)) ) * gg * V_post_tech
            # A += - np.exp(  L_mat - np.log(448) ) * gg
            
            D += xi_g * np.exp((L_mat - np.log(448))) * (1 - np.exp( - xi_g * (V_post_tech - v0)))
    ...
    if model == "Pre damage":
        ...
        g_composite = np.exp(- 1 / xi_g * ( v_i - v0 + V_post_tech - v0 ))
        
    elif model == "Post damage":
        
        g_tech = np.exp(-1 / xi_g * (V_post_tech - v0))
        # gg[gg <=1e-16] = 1e-16
        # D += xi_g * np.exp((L_mat - np.log(448))) * (1 - gg + gg * np.log(gg)) + np.exp( (L_mat - np.log(448)) ) * gg * V_post_tech
        # A += - np.exp(  L_mat - np.log(448) ) * gg
