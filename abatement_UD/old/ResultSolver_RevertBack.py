...
def _FOC_partialupdate(v0, steps= (), states = (), args=(), controls=(), fraction=0.5):
    ...
    A   = - delta * np.ones(K_mat.shape) - np.exp(  L_mat - np.log(448*1000) ) * g_tech
    ...
    D = delta * np.log(consumption) + delta * K_mat  - dG * np.sum(theta_ell * pi_c, axis=0) * ee  - 0.5 * ddG * sigma_y**2 * ee**2  + xi_a * entropy + xi_g * np.exp((L_mat - np.log(448*1000))) * (1 - g_tech + g_tech * np.log(g_tech)) + np.exp( (L_mat - np.log(448*1000)) ) * g_tech * V_post_tech
