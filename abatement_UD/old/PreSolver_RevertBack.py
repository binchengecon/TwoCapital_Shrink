...
def _FOC_update(v0, steps= (), states = (), args=(), controls=(), fraction=0.5):
    ...
    A   = - delta * np.ones(K_mat.shape) - np.exp(  L_mat - np.log(448*1000) ) * gg
    ...
    D = delta * np.log(consumption) + delta * K_mat  - dG * np.sum(theta_ell * pi_c, axis=0) * ee  - 0.5 * ddG * sigma_y**2 * ee**2  + xi_a * entropy + xi_g * np.exp((L_mat - np.log(448*1000))) * (1 - gg + gg * np.log(gg)) + np.exp( (L_mat - np.log(448*1000)) ) * gg * V_post_tech
