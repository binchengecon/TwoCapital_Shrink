...
from src.PostSolver_MA import hjb_post_damage_post_tech, hjb_pre_damage_post_tech
from src.PreSolver_Corrected_MA import pde_one_interation
from src.PreSolver_Corrected_MA import hjb_pre_tech
import argparse
...
print(epsilonarr,fractionarr,maxiterarr)
...
psi_2 = np.array([temp * np.ones((nK, nY_short, nL)) for temp in psi_2])
...
model_args =(delta, alpha, theta, vartheta_bar, lambda_bar, mu_k, kappa, sigma_k, theta_ell, pi_c_o, pi_c, sigma_y, zeta, psi_0, psi_1, psi_2, sigma_g, v_tech3, gamma_1, gamma_2, gamma_3_list, y_bar, xi_a, xi_g, xi_p)
...
model_tech1_pre_damage = hjb_pre_tech(
        state_grid=(K, Y_short, L), 
        model_args=model_args, V_post_damage=v_i, 
        tol=1e-7, epsilon=epsilonarr[2], fraction=fractionarr[2], max_iter=maxiterarr[2],
        v0=np.mean(v_i, axis=0),
        smart_guess=Guess,
        )
