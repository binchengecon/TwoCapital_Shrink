...
def _FOC_update(v0, steps= (), states = (), args=(), controls=(), fraction=0.5):
    ...
    if theta == 2 and psi_1 == 1:
        ...
        e_new = np.zeros(e_star.shape)
        
        coef_order0 = (alpha - i_star - x_star) * ( G*np.sum(pi_c * theta_ell,axis=0) + F *sigma_y**2 * alpha * lambda_bar * np.exp(K_mat))
        coef_order1 = -(alpha - i_star - x_star) * F * sigma_y**2 * alpha * lambda_bar * np.exp(K_mat)
        coef_order2 = delta * theta* vartheta_bar /(lambda_bar*np.exp(K_mat))
        coef_order3 = -alpha * vartheta_bar * (G*np.sum(pi_c*theta_ell,axis=0)+F*sigma_y**2 * alpha*lambda_bar*np.exp(K_mat))
        coef_order4 = alpha*vartheta_bar*F*sigma_y**2*alpha*lambda_bar*np.exp(K_mat)
        
        i_nonl_mat = i_star + x_star
        k_nonl_mat = K_mat
        coef_e1_mat = G*np.sum(pi_c*theta_ell,axis=0)
        coef_e2_mat = F * sigma_y**2/2
         
        coef_order0_1d = coef_order0.ravel(order='F')
        coef_order1_1d = coef_order1.ravel(order='F')
        coef_order2_1d = coef_order2.ravel(order='F')
        coef_order3_1d = coef_order3.ravel(order='F')
        coef_order4_1d = coef_order4.ravel(order='F')
        
        i_nonl_mat_1d = i_nonl_mat.ravel(order='F')
        k_nonl_mat_1d = k_nonl_mat.ravel(order='F')
        coef_e1_mat_1d = coef_e1_mat.ravel(order='F')
        coef_e2_mat_1d = coef_e2_mat.ravel(order='F')
        
        e_new_1d = e_new.ravel(order='F')
        
        def root_max(p0,p1,p2,p3,p4,i_nonl, k_nonl, coef_e1, coef_e2):
            coeff = [p4,p3,p2,p1,p0]
            root = np.roots(coeff)
            
            root_real = root[np.isreal(root)]
            root_real = root_real[root_real<=1]
            root_real = root_real[root_real>0]
            
            # max_temp = -100000000
            # if len(root_real)==0:
            #     # val0 = delta*np.log(alpha-i_nonl-alpha*vartheta_bar)
            #     # val1 = delta*np.log(alpha-i_nonl-alpha*vartheta_bar*(1e-16)**theta)+coef_e1*(1-1e-16)*alpha*lambda_bar*np.exp(k_nonl) +coef_e2*(1-(1e-16))**2
            #     e_return = 1e-16
            # elif len(root_real)>=1:
            #     e_return = 1e-16
            #     for index in range(len(root_real)):
            #         em_cand = root_real[index]
            #         e_cand = alpha*lambda_bar*np.exp(k_nonl)*(1-em_cand)
            #         val = delta*np.log(alpha-i_nonl-alpha*vartheta_bar*em_cand**theta)+coef_e1*e_cand +coef_e2*e_cand**2
            #         if val>max_temp:
            #             max_temp = val
            #             e_return = e_cand
            # return e_return
            ...
            if len(root_real)==0:
                e_return = alpha*lambda_bar*np.exp(k_nonl)
                val0 = delta*np.log(alpha-i_nonl-alpha*vartheta_bar)
                val1 = delta*np.log(alpha-i_nonl)+coef_e1*alpha*lambda_bar*np.exp(k_nonl) +coef_e2*(alpha*lambda_bar*np.exp(k_nonl))**2
                if val0>=val1:
                    e_return = 1e-16
                elif val1>val0:
                    e_return = alpha*lambda_bar*np.exp(k_nonl)
        ...
        for i_temp in range(len(coef_order4_1d)):
            e_new_1d[i_temp] = root_max(coef_order0_1d[i_temp], coef_order1_1d[i_temp], coef_order2_1d[i_temp], coef_order3_1d[i_temp], coef_order4_1d[i_temp], i_nonl_mat_1d[i_temp], k_nonl_mat_1d[i_temp], coef_e1_mat_1d[i_temp], coef_e2_mat_1d[i_temp])
            
        e_new = e_new_1d.reshape(i_star.shape,order='F')

        j_star = alpha * vartheta_bar * (1 - e_new / (alpha * lambda_bar * np.exp(K_mat)))**theta
        # j_star = alpha * vartheta_bar * (1 - e_star / (alpha * lambda_bar * np.exp(K_mat)))**theta
        j_star[j_star <= 1e-16] = 1e-16
