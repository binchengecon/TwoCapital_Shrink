...
def _FOC_update(v0, steps= (), states = (), args=(), controls=(), fraction=0.5):
    ...
    # Check before Cobweb 
    ME = - dY * np.sum(pi_c * theta_ell, axis=0) - ddY * sigma_y**2 * e_star + dG * np.sum(theta_ell * pi_c, axis=0) +  ddG * sigma_y**2 * e_star
    jj = alpha * vartheta_bar * (1 - e_star / (alpha * lambda_bar * np.exp(K_mat)))**theta
    
    jj[jj <= 1e-16] = 1e-16
    consumption = alpha - i_star - jj - x_star
    ME_total = delta/ consumption  * alpha * vartheta_bar * theta * (1 - e_star / ( alpha * lambda_bar * np.exp(K_mat)))**(theta - 1) /( alpha * lambda_bar * np.exp(K_mat) )
    pi_c_old = pi_c

    print("Cobweb before Converge: log(ME_total/ME) = [{},{}] ".format(np.min(abs(np.log(ME_total / ME))), np.max(abs(np.log(ME_total / ME)))))
    ...
    if theta == 2 and psi_1 == 1:
        ...
        e_old = e_star
        err = 1
        while err > Cobweb_Err:
            
            j_star = alpha * vartheta_bar * (1 - e_old / (alpha * lambda_bar * np.exp(K_mat)))**theta
            j_star[j_star <= 1e-16] = 1e-16
        ...
        # Check Cobweb before update i,x
        ME = - dY * np.sum(pi_c * theta_ell, axis=0) - ddY * sigma_y**2 * e_new + dG * np.sum(theta_ell * pi_c, axis=0) +  ddG * sigma_y**2 * e_new
        jj = alpha * vartheta_bar * (1 - e_new / (alpha * lambda_bar * np.exp(K_mat)))**theta
        
        jj[jj <= 1e-16] = 1e-16
        consumption = alpha - i_star - jj - x_star
        ME_total = delta/ consumption  * alpha * vartheta_bar * theta * (1 - e_new / ( alpha * lambda_bar * np.exp(K_mat)))**(theta - 1) /( alpha * lambda_bar * np.exp(K_mat) )
        pi_c_old = pi_c

        print("Cobweb Converge: Epoch={}, log(ME_total/ME) = [{},{}] ".format(epoch, np.min(np.log(ME_total / ME)), np.max(np.log(ME_total / ME))))
        ...
        # Check Cobweb after update i,x
        ME = - dY * np.sum(pi_c * theta_ell, axis=0) - ddY * sigma_y**2 * e_new + dG * np.sum(theta_ell * pi_c, axis=0) +  ddG * sigma_y**2 * e_new
        jj = alpha * vartheta_bar * (1 - e_new / (alpha * lambda_bar * np.exp(K_mat)))**theta
        
        jj[jj <= 1e-16] = 1e-16
        consumption = alpha - i_new - jj - x_new
        ME_total = delta/ consumption  * alpha * vartheta_bar * theta * (1 - e_new / ( alpha * lambda_bar * np.exp(K_mat)))**(theta - 1) /( alpha * lambda_bar * np.exp(K_mat) )

        print("Cobweb After Update i,x: log(ME_total/ME) = [{},{}] ".format(np.min(np.log(ME_total / ME)), np.max(np.log(ME_total / ME))))
        
    ii = i_new * fraction + i_star * (1 - fraction)
    ee = e_new * fraction + e_star * (1 - fraction)
    xx = x_new * fraction + x_star * (1 - fraction)
    
    # Check Cobweb after cobweb i,e,x
    ...
    print("Cobweb after cobweb i,e,x: log(ME_total/ME) = [{},{}] ".format(np.min(np.log(ME_total / ME)), np.max(np.log(ME_total / ME))))
    ...
    # recover smooth ambiguity
    log_pi_c_ratio_oldd = - G * e_star * theta_ell / xi_a

    log_pi_c_ratio_oldd += -dL * psi_0 * x_star**psi_1 * np.exp( psi_1 * K_mat - (1-psi_2) * L_mat) / xi_a

    pi_c_ratio_oldd = log_pi_c_ratio_oldd - np.max(log_pi_c_ratio_oldd)
    pi_c_oldd = np.exp(pi_c_ratio_oldd) * pi_c_o
    # print("{},{}".format(pi_c_o.min(),pi_c_o.max()))
    # pi_c_oldd = (pi_c_oldd <= 0) * 1e-16 + (pi_c_oldd > 0) * pi_c_oldd
    pi_c_oldd = pi_c_oldd / np.sum(pi_c_oldd, axis=0)


    
    # Check Cobweb after update pi_c
    ME = - dY * np.sum(pi_c * theta_ell, axis=0) - ddY * sigma_y**2 * e_new + dG * np.sum(theta_ell * pi_c, axis=0) +  ddG * sigma_y**2 * e_new
    jj = alpha * vartheta_bar * (1 - e_new / (alpha * lambda_bar * np.exp(K_mat)))**theta
    ...
    consumption = alpha - i_new - jj - x_new
    ME_total = delta/ consumption  * alpha * vartheta_bar * theta * (1 - e_new / ( alpha * lambda_bar * np.exp(K_mat)))**(theta - 1) /( alpha * lambda_bar * np.exp(K_mat) )
    print("Emission Difference = [{}] ".format(np.max(abs(ee-e_new))))
    print("RD Difference = [{}] ".format(np.max(abs(xx-x_new))))
    print("Cap Difference = [{}] ".format(np.max(abs(ii-i_new))))
    print("Recovered Weight Difference = [{}] ".format(np.max(abs(pi_c_oldd-pi_c_old))))
    print("Weight Difference = [{}] ".format(np.max(abs(pi_c-pi_c_old))))
    print("Cobweb After Update pi_c: log(ME_total/ME) = [{},{}] ".format(np.min(np.log(ME_total / ME)), np.max(np.log(ME_total / ME))))
...
def hjb_pre_tech(
    ...
    delta, alpha, theta, vartheta_bar, lambda_bar, mu_k, kappa, sigma_k, theta_ell, pi_c_o, pi_c, sigma_y, zeta, psi_0, psi_1, psi_2, sigma_g, V_post_tech, gamma_1, gamma_2, gamma_3, y_bar, xi_a, xi_g, xi_p = model_args
