...
def _FOC_update(v0, steps= (), states = (), args=(), controls=(), fraction=0.5):
    ...
    if theta == 2 and psi_1 == 1:
        ...
        e_old = e_star
        err = 1
        while err > Cobweb_Err and epoch < 100:
        ...
        print("Cobweb Converge: Epoch={}, log(ME_total/ME) = [{},{}] ".format(epoch, np.min(np.log(abs(ME_total / ME))), np.max(np.log(abs(ME_total / ME)))))
        print("Cobweb Converge: ME_total-ME = [{},{}] ".format(np.min(abs(ME_total-ME)), np.max(abs(ME_total - ME))))
