...
actiontime=2
epsilonarraypost=(0.05) # Computation of coarse grid and psi10.8, post
...
# python_name_unit="Result_2jump_combine_color_L25_f.py"
...
for epsilonpost in ${epsilonarraypost[@]}; do
    for hXarri in "${hXarrays[@]}"; do
        ...
        action_name="2jump_step_${hXarr[0]}_${hXarr[1]}_${hXarr[2]}_LR_${epsilonpost}_ah"

        for PSI_0 in ${psi0arr[@]}; do
            for PSI_1 in ${psi1arr[@]}; do

                mkdir -p ./job-outs/${action_name}/PSI0_${PSI_0}_PSI1_${PSI_1}/
...
#SBATCH --output=./job-outs/${action_name}/PSI0_${PSI_0}_PSI1_${PSI_1}/graph_${server_name}.out
#SBATCH --error=./job-outs/${action_name}/PSI0_${PSI_0}_PSI1_${PSI_1}/graph_${server_name}.err
...
python3 /home/bcheng4/TwoCapital_Shrink/abatement/${python_name_unit} --dataname  ${action_name} --pdfname ${server_name} --psi0 ${PSI_0} --psi1 ${PSI_1}  --xiaarr ${xi_a[@]} --xigarr ${xi_p[@]}   --hXarr ${hXarr[@]} --Xminarr ${Xminarr[@]} --Xmaxarr ${Xmaxarr[@]} 
