...
def simulate_pre(
    ...
    ii, ee, xx, g_tech, g_damage,  gg_mean1, gg_mean2, pi_c = controls
    n_climate = len(pi_c)
...
#     v, ME_base, diff = decompose(v0, stateSpace, (K_mat, Y_mat, L_mat), (ii, ee, xx), args=args)
    ...
    mean1_func = RegularGridInterpolator(gridpoints, gg_mean1)
    mean2_func = RegularGridInterpolator(gridpoints, gg_mean2)
...
#     L_0 = -3.7
    ...
    gt_mean1   = np.zeros([pers])
    gt_mean2   = np.zeros([pers])
...
#             if pre_damage:
            ...
            Ambiguity_mean_dis_h[tm] = np.average(theta_ell + sigma_y*gt_mean1[tm],weights=pi_c_t[:,tm])
            # print(hist[0,:])
            gt_mean1[tm] = mean1_func(hist[0,:])
            gt_mean2[tm] = mean2_func(hist[0,:])
            ...
            gt_mean1[tm] = mean1_func(hist[tm-1,:])
            gt_mean2[tm] = mean2_func(hist[tm-1,:])
...
#             ME_base_t[tm] = ME_base_func(hist[tm-1, :])
            ...
            gt_mean1_mul = sigma_y*gt_mean1*1000
            gt_mean2_mul = sigma_g*gt_mean2
...
#         ME_base = ME_base_t,
        ...
        gt_mean1_mul = gt_mean1_mul,
        gt_mean2_mul = gt_mean2_mul,
        )
...
def model_solution_extraction(xi_g,psi_0,psi_1,psi_2):
        ...
        File_Dir = "xi_g_{}_psi_0_{}_psi_1_{}_" .format(xi_g,psi_0,psi_1)
        ...
        if os.path.exists(model_simul_dir_post):
            ...
            gg_mean1 = tech1["gg_mean1"]
            gg_mean2 = tech1["gg_mean2"]
            
            # g_damage = np.ones((1, nK, nY, nL))
            res = simulate_pre(grid = (K, Y_short, L), model_args = model_args, 
                                        controls = (i,e,x, g_tech, g_damage,gg_mean1, gg_mean2, pi_c), 
    ...
    for id_psi0 in range(len(psi0arr)):
        for id_psi1 in range(len(psi1arr)):
            for id_psi2 in range(len(psi2arr)):
                    ...
                    res = model_solution_extraction(xigarr[id_xiag],psi0arr[id_psi0],psi1arr[id_psi1],psi2arr[id_psi2])
        ...
        for id_psi1 in range(len(psi1arr)):
            for id_psi2 in range(len(psi2arr)):
                ...
                if xigarr[id_xiag]>10:
                    ...
                    plt.plot(res["years"], (res["Ambiguity_mean_dis"]-res["Ambiguity_mean_undis"])*1000,label='$\\xi_m={:.3f}$' .format(xigarr[id_xiag],xigarr[id_xiag]))
        ...
        for id_psi1 in range(len(psi1arr)):
            for id_psi2 in range(len(psi2arr)):
                ...
                if xigarr[id_xiag]>10:
                    ...
                    plt.plot(res["years"], res["gt_mean1_mul"],label='baseline'.format(psi2arr[id_psi2]))
                else:
                    plt.plot(res["years"], res["gt_mean1_mul"],label='$\\xi_m={:.3f}$' .format(xigarr[id_xiag],xigarr[id_xiag]))
...
plt.savefig("./abatement/pdf_2tech/"+args.dataname+"/h1,xig={},psi0={},psi1={},psi2={}_v2_L.pdf".format(xigarr,psi0arr,psi1arr,psi2arr))
plt.savefig("./abatement/pdf_2tech/"+args.dataname+"/h1,xig={},psi0={},psi1={},psi2={}_v2_L.png".format(xigarr,psi0arr,psi1arr,psi2arr))
plt.close()
        ...
        for id_psi1 in range(len(psi1arr)):
            for id_psi2 in range(len(psi2arr)):
                ...
                if xigarr[id_xiag]>10:
                    ...
                    plt.plot(res["years"][res["states"][:, 1]<1.5], res["gt_mean1_mul"][res["states"][:, 1]<1.5],label='baseline'.format(psi2arr[id_psi2]))
                else:
                    plt.plot(res["years"][res["states"][:, 1]<1.5], res["gt_mean1_mul"][res["states"][:, 1]<1.5],label='$\\xi_m={:.3f}$' .format(xigarr[id_xiag],xigarr[id_xiag]))
                ...
                gt_mean1_mul_df = pd.DataFrame(res["gt_mean1_mul"])
                gt_mean1_mul_df.to_csv('./data/gg_mean1,xig={},psi0={},psi1={},psi2={}.csv'.format(xigarr[id_xiag],psi0arr[0],psi1arr[0],psi2arr[0]), sep='\t',header=None,index=None)


# plt.legend(loc='upper left')


plt.savefig("./abatement/pdf_2tech/"+args.dataname+"/h1,xig={},psi0={},psi1={},psi2={},BC_v2_L.pdf".format(xigarr,psi0arr,psi1arr,psi2arr))
plt.savefig("./abatement/pdf_2tech/"+args.dataname+"/h1,xig={},psi0={},psi1={},psi2={},BC_v2_L.png".format(xigarr,psi0arr,psi1arr,psi2arr))
plt.close()
