...
from src.solver_drs_unit_ambplus_ah import pde_one_interation
from src.solver_drs_unit_ambplus_ah import hjb_pre_tech
import argparse
...
xi_m = args.xi_p  # Technology jump
...
File_Name = "xi_g_{}_psi_0_{}_psi_1_{}_" .format(xi_g,psi_0,psi_1)
...
theta_ell_mean = np.mean(theta_ell)*np.ones(len(theta_ell))
theta_ell = np.array([temp * np.ones((nK, nY_short)) for temp in theta_ell_mean])
psi_2_mean = np.mean(psi_2)* np.ones(len(psi_2))
# theta_ell = np.array([temp * np.ones((nK, nY_short)) for temp in theta_ell])
psi_2 = np.array([temp * np.ones(K_mat.shape) for temp in psi_2_mean])
...
theta_ell_mean = np.mean(theta_ell)

theta_ell = theta_ell_mean * np.ones((nK, nY_short, nL))
psi_2_mean = np.mean(psi_2)

psi_2 = psi_2_mean * np.ones((nK, nY_short, nL))
...
model_args =(delta, alpha, theta, vartheta_bar, lambda_bar, mu_k, kappa, sigma_k, theta_ell, pi_c_o, pi_c, sigma_y, zeta, psi_0, psi_1, psi_2, sigma_g, v_tech2, gamma_1, gamma_2, gamma_3_list, y_bar, xi_g, xi_p, xi_m)
