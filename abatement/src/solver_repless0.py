...
def _FOC_update(v0, epoch,steps= (), states = (), args=(), controls=(), fraction=0.5):
    ...
    theta_ell_vec = theta_ell[:,0,0,0].copy()
    ...
    start_ksp1 = time.time()

    log_pi_c_ratio = - G * ee * theta_ell / xi_a

    pi_c = np.exp(log_pi_c_ratio)

    pi_c_sum = np.zeros((1,G.shape[0],G.shape[1],G.shape[2]))
    for i in range(len(theta_ell_vec)):
        log_pi_c_ratio_temp = - G * ee * theta_ell_vec[i] / xi_a
        # start_ksp2 = time.time()
        pi_c_sum[0,:,:,:] += np.exp(log_pi_c_ratio_temp)
        # start_ksp3 = time.time()
        # pi_c = (pi_c <= 0) * 1e-16 + (pi_c > 0) * pi_c
    pi_c = pi_c / pi_c_sum[0,:,:,:]
    
    start_ksp5 = time.time()

    if epoch%1000==0:
        print("linecomputation={:f}".format(start_ksp5-start_ksp1))
    # Technology
...
def hjb_pre_tech(
    ...
    while FC_Err > tol and epoch < max_iter:
        if epoch%1000==0:
            ...
            print("-----------------------------------")
        start_ep = time.time()
        A, B_1, B_2, B_3, C_1, C_2, C_3, D, dX1, dX2, dX3, ddX1, ddX2, ddX3, ii, ee, xx, pi_c = _FOC_update(v0, epoch,steps= (hX1, hX2, hX3), states = (K_mat, Y_mat, L_mat), args=FOC_args, controls=(i_star, e_star, x_star), fraction=fraction)
