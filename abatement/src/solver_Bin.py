...
def _FOC_update(v0, steps= (), states = (), args=(), controls=(), fraction=0.5):
    ...
    if theta == 2 and psi_1 == 1:
        ...
        while Converged == 0:
            ...
            j_new = alpha * vartheta_bar * (1 - e_new / (alpha * lambda_bar * np.exp(K_mat)))**theta
            j_new[j_new <= 1e-16] = 1e-16                
            consumption_new = alpha - i_new - j_new - x_new
            consumption_new[consumption_new <= 1e-16] = 1e-16
            mc_new =  delta / consumption_new

            if np.max(abs(mc_new - mc) / 0.05) <= 1e-5:
                Converged = 1
                mc = mc_new
                i_star = i_new
                e_star = e_new
            ...
            nums += 1
        print('Cobweb Passed, iterations: {}'.format(nums))
