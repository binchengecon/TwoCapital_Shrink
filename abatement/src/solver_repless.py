...
def pde_one_interation(ksp, petsc_mat, X1_mat_1d, X2_mat_1d, X3_mat_1d, lowerLims, upperLims, dVec, increVec, v0, A, B_1, B_2, B_3, C_1, C_2, C_3, D, tol, epsilon, epoch):
...
def _FOC_update(v0, epoch, steps=(), states=(), args=(), controls=(), fraction=0.5):
    ...
    if theta == 2 and psi_1 == 1:
        ...
        Omega_2 = mc * temp2 / (alpha * lambda_bar *
                                np.exp(K_mat)) - F * sigma_y**2
        e_new = Omega_1 / Omega_2
        # e_new[e_new <= 1e-15] = 1e-15
        i_new = (1 - mc / dK) / kappa
        # i_new[i_new <= 1e-15] = 1e-15
        temp3 = alpha - ii - alpha * vartheta_bar * \
            (1 - ee / (alpha * lambda_bar * np.exp(K_mat)))**theta
        ...
        i_new = (1 - mc / dK) / kappa
        j_star = alpha * vartheta_bar * \
            (1 - e_star / (alpha * lambda_bar * np.exp(K_mat)))**theta
        ...
        x_new = (mc / (dL * psi_0 * psi_1) *
                 np.exp(psi_1 * (L_mat - K_mat)))**(1 / (psi_1 - 1))
    ...
    log_pi_c_ratio = - G * ee * theta_ell / xi_a
    # start_ksp2 = time.time()
    pi_c = np.exp(log_pi_c_ratio)
    # start_ksp3 = time.time()
    # pi_c = (pi_c <= 0) * 1e-16 + (pi_c > 0) * pi_c
    pi_c = pi_c / np.sum(pi_c, axis=0)
    # Technology
    ...
    B_3 = - zeta + psi_0 * (xx * np.exp(K_mat - L_mat)
                            )**psi_1 - 0.5 * sigma_g**2
    C_1 = 0.5 * sigma_k**2 * np.ones(K_mat.shape)
    C_2 = 0.5 * sigma_y**2 * ee**2
    C_3 = 0.5 * sigma_g**2 * np.ones(K_mat.shape)
    D = delta * np.log(consumption) + delta * K_mat - dG * np.sum(theta_ell * pi_c, axis=0) * ee - 0.5 * ddG * \
        sigma_y**2 * ee**2 + xi_g * \
        np.exp(L_mat) * (1 - gg + gg * np.log(gg)) + \
        np.exp(L_mat) * gg * V_post_tech
...
def hjb_pre_tech(
    ...
    if isinstance(gamma_3, (np.ndarray, list)):
        ...
        r_2 = 2.5
        Intensity = r_1 * \
            (np.exp(r_2 / 2 * (Y_mat - y_bar_lower)**2) - 1) * (Y_mat > y_bar_lower)
        v_i = V_post_damage
        dG = gamma_1 + gamma_2 * Y_mat
        ddG = gamma_2
    else:
        model = "Post damage"
        dG = gamma_1 + gamma_2 * Y_mat + gamma_3 * \
            (Y_mat - y_bar) * (Y_mat > y_bar)
    ...
    FOC_args = (delta, alpha, theta, vartheta_bar, lambda_bar, mu_k, kappa, sigma_k, theta_ell,
                pi_c_o, pi_c, sigma_y, zeta, psi_0, psi_1, sigma_g, V_post_tech, dG, ddG, xi_a, xi_g)
    ...
    while FC_Err > tol and epoch < max_iter:
        if epoch % 1000 == 0:
            ...
            print("-----------------------------------")
        start_ep = time.time()
        A, B_1, B_2, B_3, C_1, C_2, C_3, D, dX1, dX2, dX3, ddX1, ddX2, ddX3, ii, ee, xx, pi_c = _FOC_update(v0, epoch, steps=(
            hX1, hX2, hX3), states=(K_mat, Y_mat, L_mat), args=FOC_args, controls=(i_star, e_star, x_star), fraction=fraction)

        if model == "Pre damage":
            D -= xi_p * Intensity * \
                (np.sum(pi_d_o * np.exp(- v_i / xi_p), axis=0) -
                 np.exp(- v0 / xi_p)) / np.exp(- v0 / xi_p)

        out_comp = pde_one_interation(
            ksp,
            petsc_mat, X1_mat_1d, X2_mat_1d, X3_mat_1d,
            lowerLims, upperLims, dVec, increVec,
            v0, A, B_1, B_2, B_3, C_1, C_2, C_3, D, 1e-13, epsilon, epoch)
        # if epoch % 1 == 0 and reporterror:
        # Calculating PDE error and False Transient error
        PDE_rhs = A * v0 + B_1 * dX1 + B_2 * dX2 + B_3 * \
            dX3 + C_1 * ddX1 + C_2 * ddX2 + C_3 * ddX3 + D
        PDE_Err = np.max(abs(PDE_rhs))
        FC_Err = np.max(abs((out_comp - v0) / epsilon))
        if epoch % 1000 == 0:

            print("Epoch {:d} (PETSc): PDE Error: {:.10f}; False Transient Error: {:.10f}" .format(
                epoch, PDE_Err, FC_Err), flush=True)
