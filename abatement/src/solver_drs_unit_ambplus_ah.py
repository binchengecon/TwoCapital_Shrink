...
def _FOC_update(v0, steps= (), states = (), args=(), controls=(), fraction=0.5):
    ...
    delta, alpha, theta, vartheta_bar, lambda_bar, mu_k, kappa, sigma_k, theta_ell, pi_c_o, pi_c, sigma_y, zeta, psi_0, psi_1, psi_2, sigma_g, V_post_tech, dG, ddG, xi_g, xi_m = args
    ...
    gg_mean1 = - G * sigma_y * ee / xi_m
    gg_mean2 = - dL * sigma_g / xi_m

    gg_mean1[gg_mean1<=1e-16] = 1e-16
    gg_mean1[gg_mean1>=1] = 1

    # gg_mean2[gg_mean2<=1e-16] = 1e-16
    # gg_mean2[gg_mean2>=1] = 1

    print("gg_mean1_min,max=({},{})" .format(gg_mean1.min(),gg_mean1.max()))
    print("gg_mean2_min,max=({},{})" .format(gg_mean2.min(),gg_mean2.max()))
    ...
    B_2 = (theta_ell+ sigma_y * gg_mean1) * ee
    # B_3 = - zeta + psi_0 * (xx * np.exp(K_mat - L_mat))**psi_1 - 0.5 * sigma_g**2
    B_3 = - zeta + psi_0 * xx** psi_1 * np.exp( psi_1 * K_mat ) * np.sum(pi_c * np.exp( -( 1-psi_2) * L_mat  ), axis=0 )- 0.5 * sigma_g**2 + sigma_g*gg_mean2

    C_1 = 0.5 * sigma_k**2 * np.ones(K_mat.shape)
    C_2 = 0.5 * sigma_y**2 * ee**2
    C_3 = 0.5 * sigma_g**2 * np.ones(K_mat.shape)
    D = delta * np.log(consumption) + delta * K_mat  - dG * (theta_ell + sigma_y * gg_mean1) * ee  - 0.5 * ddG * sigma_y**2 * ee**2  + xi_g * np.exp((L_mat - np.log(448))) * (1 - gg + gg * np.log(gg)) + np.exp( (L_mat - np.log(448)) ) * gg * V_post_tech + xi_m/2*( gg_mean1**2 + gg_mean2**2)

    return A, B_1, B_2, B_3, C_1, C_2, C_3, D, dX1, dX2, dX3, ddX1, ddX2, ddX3, ii, ee, xx, gg_mean1, gg_mean2
...
def hjb_pre_tech(
    ...
    delta, alpha, theta, vartheta_bar, lambda_bar, mu_k, kappa, sigma_k, theta_ell, pi_c_o, pi_c, sigma_y, zeta, psi_0, psi_1, psi_2, sigma_g, V_post_tech, gamma_1, gamma_2, gamma_3, y_bar, xi_g, xi_p, xi_m = model_args
    ...
    FOC_args = (delta, alpha, theta, vartheta_bar, lambda_bar, mu_k, kappa, sigma_k, theta_ell, pi_c_o, pi_c, sigma_y, zeta, psi_0, psi_1, psi_2, sigma_g, V_post_tech, dG, ddG, xi_g, xi_m )
    ...
    while FC_Err > tol and epoch < max_iter:
        ...
        print("-----------------------------------")
        start_ep = time.time()
        A, B_1, B_2, B_3, C_1, C_2, C_3, D, dX1, dX2, dX3, ddX1, ddX2, ddX3, ii, ee, xx, gg_mean1, gg_mean2 = _FOC_update(v0, steps= (hX1, hX2, hX3), states = (K_mat, Y_mat, L_mat), args=FOC_args, controls=(i_star, e_star, x_star), fraction=fraction)
