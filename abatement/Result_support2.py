...
def simulate_econ(
    ...
    for tm in range(pers):
        ...
        print("time={}, K={:.3f},Y={:.3f},L={:.3f},mu_K={:.3f},mu_Y={:.3f},mu_L={:.3f},ii={:.3f},ee={:.3f},xx={:.3f}" .format(tm, hist[tm,0],hist[tm,1],hist[tm,2],mu_K_hist[tm],beta_f * e_hist[tm],mu_L_hist[tm],i_hist[tm],e_hist[tm],x_hist[tm]), flush=True)
...
def simulate_UD(
    ...
    print("K=[{:.1f},{:.1f},{:.2f},{:d}], Y=[{:.1f},{:.1f},{:.2f},{:d}],L==[{:.1f},{:.1f},{:.2f},{:d}],Y_short=[{:.1f},\t{:.1f},{:.2f},{:d}]" .format(K.min(),K.max(),hK,nK, Y.min(),Y.max(),hY,nY, L.min(),L.max(),hL,nL,max(Y_short), min(Y_short),hY,nY_short ))
    ...
    File_Dir_Support_full =  "xi_a_support_{}_xi_g_support_{}_theta_ell_type_{}_".format(xi_a_support, xi_g_support, theta_ell_type)
    
    v_i_base = []
    
    for i in range(num_gamma):
        
        Model_tech2postdamage_Name = "model_tech2_post_damage_gamma_{:.4f}".format(gamma_3_list[i])

        print("------------Post damage, Tech II: gamma={:.4f}----------".format(gamma_3_list[i]))
        print("Load Success")
        model_tech2_post_damage = pickle.load(open(Data_Dir + File_Dir + File_Dir_Support_full + Model_tech2postdamage_Name, "rb"))
        ...
        print("Load Success")
        Model_tech1postdamage_Name = "model_tech1_post_damage_gamma_{:.4f}".format(gamma_3_list[i])
        
        model_tech1_post_damage = pickle.load(open(Data_Dir + File_Dir + File_Dir_Support_full+ Model_tech1postdamage_Name, "rb"))

        v_post_damage_i = model_tech1_post_damage["v0"]
    ...
    print("---------log(ME_total)=[{:.3f},{:.3f}]---------------".format(np.log(ME_total).min(), np.log(ME_total).max()), flush=True)
    # base
    print("-------------Base Start--------------")
    ...
    with open(Data_Dir+ File_Dir  + "model_tech1_pre_damage_base", "wb") as f:
        pickle.dump(res_base, f)
    with open(Data_Dir+ File_Dir  + "model_tech1_pre_damage_base2", "wb") as f:
        pickle.dump(res_base2, f)
        
    # res_base = pickle.load(open(Data_Dir+ File_Dir  + "model_tech1_pre_damage_base_scheme2", "rb"))
    # res_base2 = pickle.load(open(Data_Dir+ File_Dir  + "model_tech1_pre_damage_base2_scheme2", "rb"))
    ...
    print("---------log(ME_base)=[{:.3f},{:.3f}]---------------".format(np.log(res_base["ME"]).min(), np.log(res_base["ME"]).max()), flush=True)
    print("---------log(ME_base2)=[{:.3f},{:.3f}]---------------".format(np.log(res_base2["ME"]).min(), np.log(res_base2["ME"]).max()), flush=True)
    ...
    # Y_short = Y_short[:n_bar2+1]
    # ii = res_base2["i_star"]
    # ee = res_base2["e_star"]
    # xx = res_base2["x_star"]
    # ME_base = res_base2["ME"]
    # ME_total = ME_total[:,:n_bar2+1,:]
    # years  = np.arange(T0, T0 + T + dt, dt)
    # pers   = len(years)
    ...
    for tm in range(pers):
        ...
        print("time={}, K={:.3f},Y={:.3f},L={:.3f},mu_K={:.3f},mu_Y={:.3f},mu_L={:.3f},ii={:.3f},ee={:.3f},xx={:.3f},ME_total_base={:.3f}" .format(tm, hist[tm,0],hist[tm,1],hist[tm,2],mu_K_hist[tm],beta_f * e_hist[tm],mu_L_hist[tm],i_hist[tm],e_hist[tm],x_hist[tm],np.log(ME_total_hist[tm]/ME_base_hist[tm])*100), flush=True)
...
def model_simulation_graph(grid=(),
    ...
    plt.plot(res["years"], res["ME_total"],label='ME_total' ,linewidth=5.0)
    ...
    plt.savefig("./abatement/pdf_2tech/"+args.dataname+"/ME_total,xia={},xig={},psi0={},psi1={},psi2={}.pdf".format(xiaarr,xigarr,psi0arr,psi1arr,psi2arr))
