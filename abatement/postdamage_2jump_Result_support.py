...
from src.solver_newemission import pde_one_interation
from src.solver_newemission import hjb_pre_tech
import argparse
...
parser.add_argument("--xi_a_support", type=float, default=1000.)
parser.add_argument("--xi_g_support", type=float, default=1000.)
...
xi_a_support = args.xi_a_support  # Smooth ambiguity
xi_g_support = args.xi_g_support  # Technology jump
xi_p_support = args.xi_g_support # Hold place for arguments, no real effects 

theta_ell_type = args.theta_ell_type
...
print("-------------------------------------------")
print("------------Compute Support Value Functions:----------")
print("-----------For: xi_a={:.5f}, xi_p={:.3f}, xi_g={:.3f}--------------------".format(xi_a,xi_g,xi_g))
print("-----------At: xi_a_support={:.5f}, xi_p_support={:.3f}, xi_g_support={:.3f}, theta_ell_type={}--------------------".format(xi_a_support,xi_g_support,xi_g_support,theta_ell_type))
...
model_args = (delta, alpha, kappa, mu_k, sigma_k, theta_ell, pi_c_o, sigma_y, xi_a_support, xi_b, gamma_1, gamma_2, gamma_3_i, y_bar, theta, lambda_bar_second, vartheta_bar_second)
...
File_Name_Support =  "xi_a_support_{}_xi_g_support_{}_theta_ell_type_{}_".format(xi_a_support, xi_g_support, theta_ell_type)

with open(Data_Dir+ File_Name + File_Name_Support+"model_tech2_post_damage_gamma_{:.4f}".format(gamma_3_i), "wb") as f:
   pickle.dump(model_tech2_post_damage, f)

model_tech2_post_damage = pickle.load(open(Data_Dir + File_Name + File_Name_Support+"model_tech2_post_damage_gamma_{:.4f}".format(gamma_3_i), "rb"))
...
theta_ell_reshape = theta_ell.reshape(n_temp, n_carb, n_RD)
psi_2_reshape = psi_2.reshape(n_temp, n_carb,n_RD)

if theta_ell_type=='full':
    theta_ell = theta_ell
    psi_2 = psi_2
elif theta_ell_type=='temp':
    theta_ell =np.mean(theta_ell_reshape, axis=(1,2))
    psi_2 =np.mean(psi_2_reshape, axis=(1,2))
...
res = hjb_pre_tech(
        ...
        model_args=(delta, alpha, theta, vartheta_bar, lambda_bar, mu_k, kappa, sigma_k, theta_ell, pi_c_o, pi_c, sigma_y, zeta, psi_0, psi_1, psi_2, sigma_g, V_post_tech2, gamma_1, gamma_2, gamma_3_i, y_bar, xi_a_support, xi_g_support, xi_p_support),
        V_post_damage=None,
...
with open(Data_Dir+ File_Name  + File_Name_Support+ "model_tech1_post_damage_gamma_{:.4f}".format(gamma_3_i), "wb") as f:
    pickle.dump(res, f)
