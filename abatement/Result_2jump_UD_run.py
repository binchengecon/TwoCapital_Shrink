...
def solve_baseline(grid=(),  args = (), n_bar=(), variable_args= (), controls = (), tol=1e-6, epsilon=0.1, max_iter=10000):
    """
    ...
    # ME_base = solve_baseline(grid, args, variable_args, controls, tol=1e-6, epsilon=0.1, max_iter=10000)
    ...
    pi_c_o = np.ones(theta_ell.shape)/len(theta_ell)
...
def minimize_pi_c(grid=(),  args = (), n_bar=(), variable_args= (), controls = (), tol=1e-6, epsilon=0.1, max_iter=10000):
    """
    compute jump model with ambiguity over climate models
    """
    ...
    # n_dmg = len(gamma_3_list)
    # n_climate = len(pi_c_orig)
    pi_c_o = np.ones(pi_c_o.shape)/len(theta_ell)
...
def minimize_dmg(grid=(),  args = (), n_bar=(),variable_args= (), controls = (), tol=1e-6, epsilon=0.1, max_iter=10000):
    """
    compute jump model with ambiguity over climate models
    """
    ...
    while FC_Err > tol and epoch < max_iter:
        ...
        C_2 = 0.5 * sigma_y**2 * ee**2
        C_3 = 0.5 * sigma_g**2 * np.ones(K_mat.shape)
        D = delta * np.log(consumption) + delta * K_mat  - dG * np.sum(theta_ell* pi_c, axis=0) * ee  - 0.5 * ddG * sigma_y**2 * ee**2 
...
def minimize_tech(grid=(),  args = (), n_bar=(),variable_args= (), controls = (), tol=1e-6, epsilon=0.1, max_iter=10000):
    """
    compute jump model with ambiguity over climate models
    """
    ...
    while FC_Err > tol and epoch < max_iter:
        ...
        C_2 = 0.5 * sigma_y**2 * ee**2
        C_3 = 0.5 * sigma_g**2 * np.ones(K_mat.shape)
        D = delta * np.log(consumption) + delta * K_mat  - dG * np.sum(theta_ell* pi_c, axis=0) * ee  - 0.5 * ddG * sigma_y**2 * ee**2  + xi_g * np.exp((L_mat - np.log(448))) * (1 - g_tech + g_tech * np.log(g_tech)) + np.exp( (L_mat - np.log(448)) ) * g_tech * V_post_tech
...
def minimize_pi_c_dmg(grid=(),  args = (), n_bar=(), variable_args= (), controls = (), tol=1e-6, epsilon=0.1, max_iter=10000):
    """
    compute jump model with ambiguity over climate models
    """
    ...
    while FC_Err > tol and epoch < max_iter:
        ...
        C_2 = 0.5 * sigma_y**2 * ee**2
        C_3 = 0.5 * sigma_g**2 * np.ones(K_mat.shape)
        D = delta * np.log(consumption) + delta * K_mat  - dG * np.sum(theta_ell* pi_c, axis=0) * ee  - 0.5 * ddG * sigma_y**2 * ee**2  + xi_a * entropy 
...
def minimize_pi_c_tech(grid=(),  args = (), n_bar=(), variable_args= (), controls = (), tol=1e-6, epsilon=0.1, max_iter=10000):
    """
    compute jump model with ambiguity over climate models
    """
...
def simulate_pre_UD(
    ...
    xi_g_base=1000
    xi_p_base=1000
    args = (delta, alpha, theta, vartheta_bar, lambda_bar, mu_k, kappa, sigma_k, sigma_y, zeta, psi_0, psi_1, sigma_g, gamma_1, gamma_2, gamma_3_list, y_bar, xi_a_base, xi_g_base, xi_p_base)
    variable_args = (theta_ell_wakeup, psi_2_wakeup)
    
    v_i_updated = []
        
    gamma_3_list = np.linspace(0,1./3.,args.num_gamma)
    
    for gamma_3_i in gamma_3_list:
        # model_args_postdmg_posttech = (delta, alpha, kappa, mu_k, sigma_k, theta_ell_wakeup, pi_c_o, sigma_y, xi_a, xi_b, gamma_1, gamma_2, gamma_3_i, y_bar, theta, lambda_bar_second, vartheta_bar_second)

        # model_tech2_post_damage = hjb_post_damage_post_tech(
        #         K_mat, Y_mat, model_args_postdmg_posttech, v0=None,
        #     epsilon=0.1, fraction=0.1 ,tol=1e-8, max_iter=20000, print_iteration=True)
        ...
        print("-------gamma_3 = {gamma_3_i}--------------")
        print("-------------------------------------------")
        print("------------Post damage, Tech II----------")
        print("-------------------------------------------")
        model_tech2_post_damage = pickle.load(open(Data_Dir+ File_Dir + "model_tech2_post_damage_gamma_{:.4f}".format(gamma_3_i), "rb"))
        ...
        model_i_ii = model_i['i_star']
        model_i_ee = model_i['e_star']
        model_i_xx = model_i['x_star']
        
        xi_a_post = 1000.
        xi_g_post = 1000.
        xi_p_post = 1000.
        
        res_temp = hjb_pre_tech_partialupdate(
                state_grid=(K_mat, Y_mat, L_mat), 
                ...
                control_fixed=(model_i_ii, model_i_ee, model_i_xx),
                V_post_damage=None,
                tol=1e-6, epsilon=0.05, fraction=0.05, 
                smart_guess=Guess, 
                max_iter=20000,
                )
        
        v_post_damage_i = res_temp["v0"]
        v_post_damage_temp = np.zeros((nK, nY, nL))
        for j in range(nY):
            v_post_damage_temp[:, j, :] = v_post_damage_i[:, id_2, :]
        v_i_updated.append(v_post_damage_temp)
        
    v_i_updated = np.array(v_i_updated)


    controls=(ii,ee,xx, g_tech, g_damage,  pi_c_o_wakeup, pi_c_wakeup, pi_d_o_wakeup, v0, v_i_updated, v_tech2)
    ME_base = solve_baseline(grid, args, n_bar1, variable_args, controls, tol=1e-6, epsilon=0.1, max_iter=10000) # n_bar free
    ME_base2 = solve_baseline(grid, args, n_bar2, variable_args, controls, tol=1e-6, epsilon=0.1, max_iter=10000) #n_bar hit 2
...
#             ME_base_t[tm] = ME_base_func(hist[tm-1, :])
    ...
    LHS = alpha * theta * vartheta_bar / (alpha * lambda_bar*np.exp(hist[:, 0])) * jt**(theta -1)
...
def model_solution_extraction(xi_a,xi_g,psi_0,psi_1,psi_2):
    ...
    if os.path.exists(model_simul_dir_post):
        ...
        res = simulate_pre_UD(grid = (K, Y, L, Y_short), model_args = model_args, 
