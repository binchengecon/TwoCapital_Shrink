...
x = data_bin["x_star"]
pi_c = data_bin["pi_c"]
g_tech = data_bin["g_tech"]
g_damage =  data_bin["g_damage"]


# g_damage = np.ones((1, nK, nY, nL))
res_bin = simulate_pre(grid = (K, Y_short, L), model_args = model_args, 
...
x = data_suri["x_star"]
pi_c = data_suri["pi_c"]
g_tech = data_suri["g_tech"]
g_damage =  data_suri["g_damage"]


# g_damage = np.ones((1, nK, nY, nL))
res_suri = simulate_pre(grid = (K, Y_short, L), model_args = model_args, 
                            controls = (i,e,x, g_tech, g_damage, pi_c), 
                            T0=0, T=IntPeriod, dt=timespan,printing=False)

plt.plot(res_suri["years"][res_suri["states"][:,1]<1.5], res_suri["x"][res_suri["states"][:,1]<1.5], 
         label="$baseline\_suri$", color="darkgreen")
plt.plot(res_bin["years"][res_bin["states"][:,1]<1.5], res_bin["x"][res_bin["states"][:,1]<1.5], 
         label="$baseline\_bin$", color="C3",linestyle="dashed")
plt.ylim(0)
plt.legend()
