...
i = data_bincode["i_star"]
e = data_bincode["e_star"]
x = data_bincode["x_star"]
pi_c = data_bincode["pi_c"]
g_tech = data_bincode["g_tech"]
g_damage =  data_bincode["g_damage"]


# g_damage = np.ones((1, nK, nY, nL))
res_bincode = simulate_pre(grid = (K, Y_short, L), model_args = model_args, 
                            controls = (i,e,x, g_tech, g_damage, pi_c), 
                            T0=0, T=IntPeriod, dt=timespan,printing=False)

i = data_suricode["i_star"]
e = data_suricode["e_star"]
x = data_suricode["x_star"]
pi_c = data_suricode["pi_c"]
g_tech = data_suricode["g_tech"]
g_damage =  data_suricode["g_damage"]


# g_damage = np.ones((1, nK, nY, nL))
res_suricode = simulate_pre(grid = (K, Y_short, L), model_args = model_args, 
                            controls = (i,e,x, g_tech, g_damage, pi_c), 
                            T0=0, T=IntPeriod, dt=timespan,printing=False)
...
plt.plot(res_suricode["years"][res_suricode["states"][:,1]<1.5], res_suricode["x"][res_suricode["states"][:,1]<1.5], 
         label="$baseline: Generated By Suri's Code$", color="darkgreen")
plt.plot(res_suriapril["years"][res_suriapril["states"][:,1]<1.5], res_suriapril["x"][res_suriapril["states"][:,1]<1.5], 
         label="$baseline: Suri's April Result$", color="darkblue")
plt.plot(res_bincode["years"][res_bincode["states"][:,1]<1.5], res_bincode["x"][res_bincode["states"][:,1]<1.5], 
         label="$baseline: Generated By Bin's Code$", color="C3",linestyle="dashed")
plt.ylim(0)
plt.legend()
plt.title("R&D investment")
plt.xlabel("Years")

plt.savefig("./abatement/pdf_2tech/comparison/mercury.pdf")