...
static inline void fill_mat_values_Upwind(PetscScalar *State, PetscInt i, PetscInt center, PetscInt j, PetscScalar *lowerLims, PetscScalar *upperLims, PetscScalar *dVec, PetscInt *incVec, PetscInt maxcols,PetscScalar *BF,PetscScalar *BB, PetscScalar *C, PetscScalar dt, PetscInt *cols, PetscScalar *vals)
{ 
  PetscScalar firstCoefEF = BF[i],firstCoefEB = BB[i],secondCoefE = C[i];
...
PetscErrorCode FormLinearSystem_C_Upwind(PetscScalar *R, PetscScalar *F, PetscScalar *K, PetscScalar *A, PetscScalar *B_rF,PetscScalar *B_rB, PetscScalar *B_fF, PetscScalar *B_fB ,PetscScalar *B_kF, PetscScalar *B_kB,PetscScalar *C_rr, PetscScalar *C_ff, PetscScalar *C_kk, PetscScalar dt, PetscScalar *lowerLims, PetscScalar *upperLims, PetscScalar *dVec, PetscInt *incVec, PetscInt n, Mat petsc_mat)
{
  PetscErrorCode ierr;
  PetscInt       i, center;
  PetscInt       cols[13];
  ...
  for (i = 0; i < n; ++i) {
    ...
    cols[center] = i;
    vals[center] = 1.0 - dt * A[i];
    fill_mat_values_Upwind(R,i,center,0,lowerLims,upperLims,dVec,incVec,n,B_rF,B_rB,C_rr,dt,cols,vals);
    fill_mat_values_Upwind(F,i,center,1,lowerLims,upperLims,dVec,incVec,n,B_fF,B_fB,C_ff,dt,cols,vals);
    fill_mat_values_Upwind(K,i,center,2,lowerLims,upperLims,dVec,incVec,n,B_kF,B_kB,C_kk,dt,cols,vals);
    ierr = MatSetValues(petsc_mat,1,&i,3*4+1,cols,vals,INSERT_VALUES);CHKERRQ(ierr);
  }
  ierr = MatAssemblyBegin(petsc_mat,MAT_FINAL_ASSEMBLY);CHKERRQ(ierr);
  ierr = MatAssemblyEnd(petsc_mat,MAT_FINAL_ASSEMBLY);CHKERRQ(ierr);
