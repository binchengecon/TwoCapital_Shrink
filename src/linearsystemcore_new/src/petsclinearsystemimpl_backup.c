#include "petsclinearsystem_backup.h"
...
// static inline void fill_mat_values_noFT(PetscScalar *State, PetscInt i, PetscInt center, PetscInt j, PetscScalar *lowerLims, PetscScalar *upperLims, PetscScalar *dVec, PetscInt *incVec, PetscInt maxcols,PetscScalar *B, PetscScalar *C, PetscScalar dt, PetscInt *cols, PetscScalar *vals)
// { 
//   PetscScalar firstCoefE = B[i],secondCoefE = C[i];
...
//     vals[center-1-2*j] = -1*-firstCoefE*(firstCoefE < 0)/dVec[j] - secondCoefE/(PetscPowReal(dVec[j],2));
//     cols[center-1-2*j] = i - incVec[j];
//     if (i + incVec[j] < maxcols) cols[center+1+2*j] = i + incVec[j]; // ignore out of bound entries
//   }
// }

PetscErrorCode FormLinearSystem_noFT_C(PetscScalar *R, PetscScalar *F, PetscScalar *K, PetscScalar *A, PetscScalar *B_r, PetscScalar *B_f, PetscScalar *B_k, PetscScalar *C_rr, PetscScalar *C_ff, PetscScalar *C_kk, PetscScalar dt, PetscScalar *lowerLims, PetscScalar *upperLims, PetscScalar *dVec, PetscInt *incVec, PetscInt n, Mat petsc_mat)
{
  PetscErrorCode ierr;
  PetscInt       i, center;
  PetscInt       cols[13];
  ...
  for (i = 0; i < n; ++i) {
    ...
    cols[center] = i;
    vals[center] = - A[i];
    fill_mat_values(R,i,center,0,lowerLims,upperLims,dVec,incVec,n,B_r,C_rr,1,cols,vals);
    fill_mat_values(F,i,center,1,lowerLims,upperLims,dVec,incVec,n,B_f,C_ff,1,cols,vals);
    fill_mat_values(K,i,center,2,lowerLims,upperLims,dVec,incVec,n,B_k,C_kk,1,cols,vals);
    ierr = MatSetValues(petsc_mat,1,&i,3*4+1,cols,vals,INSERT_VALUES);CHKERRQ(ierr);
  }
  ierr = MatAssemblyBegin(petsc_mat,MAT_FINAL_ASSEMBLY);CHKERRQ(ierr);
  ierr = MatAssemblyEnd(petsc_mat,MAT_FINAL_ASSEMBLY);CHKERRQ(ierr);
